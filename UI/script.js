...
function logIn(event) {
    if (event.target.id) {
        ...
        toMain = mainPart.querySelectorAll(".to_main");
        toMain[0].addEventListener('click', showMainPage);
...
function userCheck() {
    var left = document.querySelector('.left-header');
    var add = document.createElement('button');
    add.className = "icon";
    add.innerHTML = "<img src='images/icons/add.png'>";
    ...
    if (!user) {
        ...
        var left = document.querySelector('.left-header');
        var add = document.querySelector('.icon');
        if(left && add) {
            left.removeChild(add);
        }
    }
    else {
    ...
    sing.addEventListener('click', logIn);
...
function showAdd(event, ID="") {
    ...
    if (!ID) {
        ...
        ID = String(GLOBAL_ARTICLES[GLOBAL_ARTICLES.length-1].id + 1);
...
function a(event) {
    ...
    localStorage.setItem('authors', JSON.stringify(GLOBAL_AUTHORS));
    return;
};
...
window.addEventListener("DOMContentLoaded",function(){
    ...
    {
        var tmp = JSON.parse(localStorage.getItem('articles'));
        tmp.forEach(function(item){
            item.createdAt = new Date(item.createdAt);
        });
        GLOBAL_ARTICLES = tmp;
    }
    if (JSON.parse(localStorage.getItem('authors'))){
        GLOBAL_AUTHORS = JSON.parse(localStorage.getItem('authors'));
    }
